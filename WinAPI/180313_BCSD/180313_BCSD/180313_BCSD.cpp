// 180313_BCSD.cpp : 응용 프로그램에 대한 진입점을 정의합니다.
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	//마우스 왼쪽버튼 눌렀을 때
	case WM_LBUTTONDOWN:
		//마우스위치 lParamdp 32비트로 x, y값 붙어서 옴. LOWORD, HIWORD 사용해서 분리
		if (!g_tArea.bStart)
		{
			g_tArea.bStart = true;
			...
			//강제로 WM_PAINT 메세지 호출해주는 함수
			//(윈도우핸들, 초기화영역(NULL넣으면 전체갱신), 현재화면지우고 갱신여부)
	...
	//키 눌렸을때
	case WM_KEYDOWN:
		//이 메세지가 들어올 경우 wParam에 어떤 키를 눌렀는지 들어옴
		switch (wParam)
		{
		case VK_ESCAPE: //ESC 눌렀을 경우:
			DestroyWindow(hWnd);
			break;
		}
		break;
    ...
    case WM_PAINT:
        {
            ...
            // TODO: 여기에 hdc를 사용하는 그리기 코드를 추가합니다. (추가중..)

			//문자 출력 (출력위치, 좌표x, 좌표y, 유니코드문자열, 글자 개수)
			TextOut(hdc, 50, 50, L"win32", 5);
			TextOut(hdc, 150, 50, TEXT("win32"), 5);
			TextOut(hdc, 250, 50, L"win32", 5);
				// 유니코드 문자열은 ""앞에 L을 붙인다.
				// 아니면 TEXT 매크로 이용. TEXT(" ")
			...
			wsprintf(strMouse, TEXT("x : %d y : %d"), g_tArea.ptStart.x,
				g_tArea.ptStart.y);
			TextOut(hdc, 600, 30, strMouse, lstrlen(strMouse)); //유니코드 문자열 길이

			//사각형 출력 (위치, 왼쪽위, 오른아래)
			Rectangle(hdc, 100, 100, 200, 200);

			//원 그리기(위치, 시작점, 끝점) - 사각형 안
